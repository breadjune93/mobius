...
from app.api.v1.schemas.nexus_schema import *
from app.api.v1.services.nexus_service import NexusService
from app.db.repositories.nexus_repository import NexusRepository

router = APIRouter(prefix="/api/v1/nexus", tags=["nexus"])
service = NexusService(NexusRepository())
...
def get_pylons(db: Session = Depends(get_db), user_id: str = Depends(get_current_subject)):
    print(f"pylons user_id: {user_id}")
